...
from routine.models import Routine, RoutineExercise, UserExerciseProgress, ExerciseSetLog, RoutineProgress, Exercise, WorkoutSession
from users.models import TrainerClientRelation
from django.utils import timezone
from datetime import timedelta

class Command(BaseCommand):
    help = 'Assigns a 7-day routine (3 sets per exercise) to user obadax12 from trainer bdfb, and marks full progress with workout sessions.'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            try:
                ...
                return
        try:
            client = User.objects.get(username='obadax12')
        except User.DoesNotExist:
            try:
                client = User.objects.get(email='ob@gmail.com')
            except User.DoesNotExist:
                self.stdout.write(self.style.ERROR("Client 'obadax12' or 'ob@gmail.com' not found."))
                return
        ...
        routine_name = "1-Week 3-Set Routine (obadax12)"
        ...
        # 6. For each day, start a WorkoutSession, log all sets, then complete the session
        today = timezone.now().date()
        for day in range(1, 8):
            session = WorkoutSession.objects.create(
                user=client,
                routine=routine,
                start_time=timezone.now() + timedelta(days=day-1),
                status='active'
            )
            ...
            session.end_time = session.start_time + timedelta(minutes=40)
            session.status = 'completed'
            session.save()
        self.stdout.write(self.style.SUCCESS(f"Assigned and marked full 3-set progress with workout sessions for user {client.username} on routine '{routine.name}' for 7 days!")) 