...
UNIT_RULES = [
    ...
    (r"(soy sauce|vinegars?|hot sauce|mustard|ketchup)\b", "tbsp", 15, 1, 2),
    (r"(sauces?|dressings?|mayo|jell(y|ies)|jams?|syrups?|honey)\b", "tbsp", 20, 1, 2),
    # --- fats -----------------------------------------------------------------
    (r"(almond butter|peanut butter|tahini)\b", "tbsp", 16, 1, 2),
...
ACCOMPANIMENT_PATTERN = re.compile(
    r"(oils?|butter|ghee|honey|tahini|pickles?|olives?)\b")


def unit_for(name: str):
    low = (name or "").lower()
    for pattern, unit, grams, lo, hi in UNIT_RULES:
